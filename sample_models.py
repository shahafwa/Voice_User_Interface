...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    bn = BatchNormalization(name='bn_rnn_1d_0')(simp_rnn)
    rnn_next = bn
    
    for i in range(recur_layers - 1):
        rnn_name = 'rnn_{}'.format(i+1)
        simp_rnn = GRU(units, activation='relu',
        return_sequences=True, implementation=2, name=rnn_name)(rnn_next)
    ...
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(rnn_next)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
